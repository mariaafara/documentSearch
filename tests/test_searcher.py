...
def test_searcher():
    ...
    assert (
            DeepDiff(searcher.search(query_terms=["token1"]), [("id_1", ["token1"])], ignore_order=True) == {}
    )
    assert DeepDiff(searcher.search(query_terms=["token8"]), [], ignore_order=True, ) == {}
    assert (
            DeepDiff(
                searcher.search(query_terms=["token5"], ),
