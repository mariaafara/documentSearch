...
class InMemoryIndexStore(IndexStore):
    ...
    def __init__(self):
        ...
        self.indices_dict = defaultdict(
            lambda: defaultdict(set))  # nested defaultdict {ngram: { 'embedding' :{[]}, 'doc_ids': {[]}}}

    def add_doc(self, doc_id, tokens) -> None:
        for token in tokens:
            self.indices_dict[token]["doc_ids"].add(doc_id)
    ...
    def get_docs(self, tokens: List[str]) -> List[Tuple[str, List[str]]]:
        ...
        for token in tokens:
            docs_ids = self.indices_dict[token]["doc_ids"]
